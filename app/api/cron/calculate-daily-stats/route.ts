...
export async function GET(request: Request) {
  try {
    ...
    const { count: scheduledCompleted } = await supabaseClient
    ...
    const { count: scheduledFailed } = await supabaseClient
      .from("scheduled_messages")
      .select("*", { count: "exact", head: true })
      .eq("status", "failed")
      .gte("processed_at", yesterdayISO)
    ...
    const { error: insertError } = await supabaseClient.from("daily_statistics").insert({
      date: yesterdayDate,
